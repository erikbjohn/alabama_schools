...
The raw school file need to be gathered.

```{r schoolsGather}
dt <- data.table::fread('~/Dropbox/pkg.data/alabama_schools/raw/Failing Schools Panel.csv', header = TRUE)
dt.cols <- which(names(dt) %in% c('School Name', 'School District', 'County Population', 'Median Per Capita Income', as.character(2002:2017)))
dt <- dt[, dt.cols, with=FA]
